...
-- Script starts here
local helicam = false
local polmav_hash = GetHashKey("police_heli")
local armyheli_hash = GetHashKey("firehawk")
local fov = (fov_max+fov_min)*0.5
local vision_state = 0 -- 0 is normal, 1 is nightmode, 2 is thermal vision
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if IsPlayerInArmyheli() then
			local aPed = GetPlayerPed(-1)
			local heli = GetVehiclePedIsIn(aPed)
			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
					Citizen.Trace("로프를 내리기를 시전 합니다.")
					if GetPedInVehicleSeat(heli, 1) == aPed or GetPedInVehicleSeat(heli, 2) == aPed then
						PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
						TaskRappelFromHeli(GetPlayerPed(-1), 1)
					else
						SetNotificationTextEntry( "STRING" )
						AddTextComponentString("~r~해당 위치에서 로프를 꺼낼 수 없습니다.")
...
AddEventHandler('heli:spotlight', function(serverID, state)
	...
	Citizen.Trace("헬리콥터 조명상태를 다음과 같이 설정합니다. "..tostring(state).." 서버 아이디: "..serverID)
end)
...
function IsPlayerInArmyheli()
	local aPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(aPed)
	return IsVehicleModel(vehicle, armyheli_hash)
end
...
function RenderVehicleInfo(vehicle)
	...
	SetTextOutline()
	SetTextEntry("STRING")
	AddTextComponentString("차량 모델 : "..vehname.."\n 번호판: "..licenseplate)
	DrawText(0.45, 0.9)
end
