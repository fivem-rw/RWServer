...
RegisterNetEvent("CarryPeople:kill")
AddEventHandler(
	"CarryPeople:kill",
	function()
		Wait(1000)
...
Citizen.CreateThread(
	function()
		...
		ClearPedSecondaryTask(GetPlayerPed(-1))
		while true do
			if not isRestControl and not isSyncing and not IsControlPressed(0, 21) and IsControlJustReleased(0, 82) then
			...
			if isEnableFree and piggyBackInProgressTarget == true and piggyBackInProgressTargetPlayer ~= nil then
				if not isSyncing and IsControlPressed(0, 21) and IsControlJustReleased(0, 82) then
					isSyncing = true
					TriggerServerEvent("CarryPeople:stop", GetPlayerServerId(piggyBackInProgressTargetPlayer))
