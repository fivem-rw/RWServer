...
function takeHostage()
	...
	if not canTakeHostage then
		drawNativeNotification("피스톨을 착용한 상태에서 사용가능합니다.")
	end
	...
	if closestPlayer ~= -1 and closestPlayer ~= nil then
		if not holdingHostageInProgress and canTakeHostage then
			if not IsEntityAttached(GetPlayerPed(closestPlayer)) and not IsEntityAttached(GetPlayerPed(-1)) then
				...
				ClearPedSecondaryTask(GetPlayerPed(closestPlayer))
		...
		drawNativeNotification("근처에 플레이어가 없습니다.")
	end
...
AddEventHandler(
	...
	function(target, animationLib, animation2, distans, distans2, height, length, spin, controlFlag, animFlagTarget, attach)
...
RegisterNetEvent("cmg3_animations:kill")
AddEventHandler(
	"cmg3_animations:kill",
	function()
		Wait(1000)
		SetEntityHealth(GetPlayerPed(-1), 0)
	end
...
Citizen.CreateThread(
	function()
		while true do
			if not beingHeldHostage and not holdingHostageInProgress and not IsControlPressed(0, 21) and IsControlJustReleased(0, 126) and not IsPedInAnyVehicle(GetPlayerPed(-1), false) then
				takeHostage()
			end
			if holdingHostage then
				...
				DrawText3D(playerCoords.x, playerCoords.y, playerCoords.z, "[G]키 놔주기 / [H]키 사살")
