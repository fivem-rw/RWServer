...
Citizen.CreateThread(
	function()
		while true do
			...
			local item = get_item(POS_actual)

			if (Vdist(playerPos.x, playerPos.y, playerPos.z, item.coords.x, item.coords.y, item.coords.z) > 0.8) then
			...
			if gui_interiors.opened then
				...
				menu.buttons = {}
				for i, dest in pairs(item.destination) do
					table.insert(menu.buttons, get_item(dest))
				end
