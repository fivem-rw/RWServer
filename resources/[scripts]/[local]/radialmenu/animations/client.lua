...
local handsUpButton = 20 --- H by default -- use 73 for X
local IsCigar = false
local cigar = nil
...
Citizen.CreateThread(
	function()
		while true do
			...
			if (IsControlJustPressed(0, radioButton)) and false then
		...
		if tostring(args[1]) == nil then
			...
			if tostring(args[1]) ~= nil then
				...
				if argh == "surrender" then
					...
					Citizen.CreateThread(
						function()
							--disabling controls while surrendured
				...
				elseif argh == "notes" then
					...
					if (DoesEntityExist(player) and not IsEntityDead(player)) then
						...
						if (IsEntityPlayingAnim(player, ad, "base", 3)) then
							...
							prop = CreateObject(RWO, GetHashKey(prop_name), x, y, z + 0.2, true, true, true)
							secondaryprop = CreateObject(RWO, GetHashKey(secondaryprop_name), x, y, z + 0.2, true, true, true)
				...
				elseif argh == "cigar" then
					...
					if (DoesEntityExist(playerPed) and not IsEntityDead(playerPed)) then
						if IsCigar then
							...
							cigar = CreateObject(RWO, GetHashKey(cigar_name), x, y, z + 0.2, true, true, true)
...
Citizen.CreateThread(
	function(prop_name, secondaryprop_name)
		while true do
			Citizen.Wait(500)
