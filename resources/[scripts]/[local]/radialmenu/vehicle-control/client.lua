...
AddEventHandler('trunk',function() 
	...
	if distanceToVeh <= interactionDistance then
		...
		ShowNotification("~r~차량과의 거리가 너무 멉니다.")
	end
end)
-- R E A R  D O O R S --
...
AddEventHandler('lock',function()
	local player = GetPlayerPed(-1)
    local vehicle = saveVehicle
	local vehicle2 = GetVehiclePedIsIn(player,true)
	if IsPedSittingInAnyVehicle(player) then
		vehicle = vehicle2
	end
	...
	local distanceToVeh = GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(vehicle), 1)
		if DoesEntityExist(vehicle) then
			if distanceToVeh <= lockDistance then
				...
				ShowNotification("차량의 문을 잠궜습니다.")
				TriggerEvent('lockLights')
				else
				SetVehicleDoorsLocked(vehicle,1)
				ShowNotification("차량의 문이 열었습니다.")
				TriggerEvent('lockLights')
				end
			else
				ShowNotification("~r~차량과의 거리가 너무 멉니다.")
			...
			ShowNotification("~r~차량의 키가 없습니다.")
		end
	end)
...
-- S A V E --
RegisterNetEvent('getvkey')
AddEventHandler('getvkey',function() 
	local player = GetPlayerPed(-1)
	if (IsPedSittingInAnyVehicle(player)) then 
		if  saved == true then
			--remove from saved.
			...
			ShowNotification("차량의 키를 ~r~버렸~w~습니다..")
			saved = false
		else
			RemoveBlip(targetBlip)
			...
			ShowNotification("해당 차량의 키를 얻었습니다. 원격 조정이 가능합니다.")
			saved = true
		end
	end
end)
RegisterNetEvent('delvkey')
AddEventHandler('delvkey',function() 
	saveVehicle = nil
	RemoveBlip(targetBlip)
	ShowNotification("차량의 키를 버렸습니다.")
	saved = false
end)
-- R E M O T E --
