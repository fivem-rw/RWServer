vRPSkateboardC = {}
Tunnel.bindInterface("vrp_skateboard", vRPSkateboardC)
Proxy.addInterface("vrp_skateboard", vRPSkateboardC)
vRP = Proxy.getInterface("vRP")
vRPSkateboardS = Tunnel.getInterface("vrp_skateboard", "vrp_skateboard")
...
RegisterCommand(
	"lb",
	function()
		vRPSkateboardS.reqPerm()
	end
)

function vRPSkateboardC.startRCCar()
	RCCar.Start()
end
	...
	Citizen.CreateThread(
		function()
			...
			if Attached then
				...
				if (-300.0 < x and x > 300.0) or (-300.0 < y and y > 300.0) or (HasEntityCollidedWithAnything(RCCar.Entity) and GetEntitySpeed(RCCar.Entity) > 2.6) then
					RCCar.AttachPlayer(false)
...
RCCar.Spawn = function()
	...
	RCCar.Entity = CreateVehicle(RWV, GetHashKey("rcbandito"), spawnCoords, spawnHeading, true)
	RCCar.Skate = CreateObject(RWO, GetHashKey("p_defilied_ragdoll_01_s"), 0.0, 0.0, 0.0, true, true, true)
	...
	RCCar.Driver = CreatePed(RWP, 5, 68070371, spawnCoords, spawnHeading, true)
