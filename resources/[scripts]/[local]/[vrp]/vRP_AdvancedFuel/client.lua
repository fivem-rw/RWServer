vRP_fuelS = Tunnel.getInterface("vRP_fuel", "vRP_fuel")
essenceS = Tunnel.getInterface("essence", "essence")
				...
				if (menu) then
					...
					TriggerEvent(
						...
						function(cb)
							if (cb) then
								menu = not menu
								essenceS.buy({int, stationNumber, false})
							else
							end
						end
					)
				...
				if (menu) then
					...
					TriggerEvent(
						...
						function(cb)
							if (cb) then
								menu = not menu
								essenceS.buy({int, electricityPrice, true})
							else
							end
						end
					)
				...
				if (menu) then
					...
					TriggerEvent(
						...
						function(cb)
							if (cb) then
								menu = not menu
								essenceS.buy({int, stationBoatNumber, false})
							else
							end
						end
					)
				...
				if (menu) then
					...
					TriggerEvent(
						...
						function(cb)
							if (cb) then
								menu = not menu
								essenceS.buy({int, stationPlaneNumber, false})
							else
							end
						end
					)
				...
				if (menu) then
					...
					TriggerEvent(
						...
						function(cb)
							if (cb) then
								menu = not menu
								essenceS.buy({int, stationHeliNumber, false})
							else
							end
						end
					)
...
function CheckVeh()
	if (IsPedInAnyVehicle(GetPlayerPed(-1)) and not isBlackListedModel()) then
		...
		if (refresh) then
			vRP_fuelS.getFuel({GetVehicleNumberPlateText(GetVehiclePedIsUsing(GetPlayerPed(-1))), GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1))))})
		...
		if (not refresh) then
			essenceS.setToAllPlayerEscense({essence, lastPlate, lastModel})
			refresh = true
		end
	end
	...
	if (IsPedInAnyVehicle(GetPlayerPed(-1), -1) and GetPedVehicleSeat(GetPlayerPed(-1)) == -1 and not isBlackListedModel()) then
		if (hud_form == 1) then
			...
			if (showText) then
				...
				DrawAdvancedText(text_x, text_y, -0.15, -0.4, 0.4, round(percent, 1) .. "%", 255, 255, 255, 255, 0, 1)
			end
		else
			if (showBar) then
				--DrawRect(hud_x, hud_y, 0.15, 0.0149999999999998, 255, 255, 255, 200)
				DrawRect(hud_x, hud_y, 0.142, 0.0119999999999998, 80, 80, 80, 150)
				DrawRect(hud_x, hud_y, essence, 0.0119999999999998, 225, 146, 45, 200)
			end

			if (showText) then
				local percent = (essence / 0.142) * 100

				DrawAdvancedText(text_x, text_y, 0.005, 0.0028, 0.25, round(percent, 2) .. "%", 255, 146, 45, 255, 0, 1)
			end
		end
	end
end
...
AddEventHandler(
	...
	function(percent, plate, model)
		...
		vRP_fuelS.setEssence_s({percent, plate, model})
	end
)
...
AddEventHandler(
	...
	function(amount)
		...
		essence.setToAllPlayerEscense({essence, GetVehicleNumberPlateText(GetVehiclePedIsUsing(GetPlayerPed(-1))), GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1))))})
