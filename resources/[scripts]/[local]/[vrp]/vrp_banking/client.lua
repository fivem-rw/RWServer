    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('chatMessage', "", {255, 255, 255}, "^5E ^0키를 눌러서 은행을 이용합니다.");
        end
        atBank = true
        if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1차안에서는 은행은 이용할 수 없습니다.");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
RegisterNUICallback('quickCash', function(data, cb)
  TriggerEvent('bank:withdraw', 1000000)
  cb('ok')
end)
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {0, 255, 0}, "^1차량에서 ATM을 사용할 수 없습니다!");
    else
      TriggerServerEvent("bank:deposit", tonumber(amount))
    end
  else
    TriggerEvent('chatMessage', "", {0, 255, 0}, "^1입금은 은행 또는 ATM에서 할 수 있습니다.");
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    ...
    TriggerEvent('chatMessage', "", {0, 255, 0}, "^1출금은 은행 또는 ATM에서 할 수 있습니다.");
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent('chatMessage', "", {0, 255, 0}, "^1이 플레이어는 온라인 상태가 아닙니다!");
    end
  else
    TriggerEvent('chatMessage', "", {0, 255, 0}, "^1이 플레이어는 근처에 있지 않습니다!");
  end
end)
