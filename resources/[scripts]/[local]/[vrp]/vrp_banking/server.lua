...
AddEventHandler(
  ...
  function(s, n, m)
    ...
    if command[1] == "/checkbalance" then
      TriggerClientEvent("chatMessage", s, "BANK", {0, 200, 0}, "계좌 잔액 확인: ^3" .. bankbalance)
      TriggerClientEvent("banking:updateBalance", source, bankbalance)
      CancelEvent()
    end
    ...
    if command[1] == "/transfer" then
      if (command[2] ~= nil and tonumber(command[3]) > 0) then
        ...
        TriggerClientEvent("chatMessage", s, "BANK", {0, 200, 0}, "^1전송 양식 /transfer [고유번호]] [금액]^0")
      end
      CancelEvent()
    end
  end
...
AddEventHandler(
  ...
  function(amount)
    ...
    if (rounded <= wallet) then
      ...
      TriggerClientEvent("chatMessage", source, "은행", {0, 200, 0}, "^5" .. amount .. "^0을 입금했습니다.")
      deposit(source, rounded)
    else
      vRPclient.notify(user_id, {"~r~계좌에 잔고가 부족합니다!"})
    end
  end
)
...
AddEventHandler(
  ...
  function(amount)
    ...
    if tonumber(amount) > 1000000000 then
      vRPclient.notify(user_id, {"~r~1회 출금금액이 너무 큽니다."})
    else
      local bankbalance = vRP.getBankMoney({user_id})
      if (tonumber(rounded) <= tonumber(bankbalance)) then
        ...
        TriggerClientEvent("chatMessage", source, "은행", {0, 200, 0}, "^5" .. amount .. "^0이 출금되었습니다.")
...
AddEventHandler(
  "bank:transfer",
  function(fromPlayer, toPlayer, amount)
    local user_id = vRP.getUserId({source})
    local fPlayer = vRP.getUserSource({user_id})

    if tonumber(fPlayer) == tonumber(toPlayer) then
      TriggerClientEvent("chatMessage", fPlayer, "", {0, 255, 0}, "^1자신에게는 보낼 수 없습니다.")
      CancelEvent()
    else
      local rounded = round(tonumber(amount), 0)
      if tonumber(amount) > 100000000 then
        TriggerClientEvent("chatMessage", fPlayer, "", {0, 255, 0}, "^11회 전송금액이 너무 큽니다!")
        CancelEvent()
      else
        local bankbalance = vRP.getBankMoney({user_id})
        if (tonumber(rounded) <= tonumber(bankbalance)) then
          ...
          TriggerClientEvent("chatMessage", fPlayer, "", {0, 255, 0}, "^0보냄: ^1" .. math.floor(rounded))
          TriggerClientEvent("chatMessage", fPlayer, "", {0, 255, 0}, "^0잔고: ^4" .. math.floor((bankbalance - rounded)))
          ...
          TriggerClientEvent("chatMessage", toPlayer, "", {0, 255, 0}, "^0받음: ^2" .. math.floor(rounded))
          TriggerClientEvent("chatMessage", toPlayer, "", {0, 255, 0}, "^0잔고: ^4" .. math.floor((bankbalance2 + rounded)))
          CancelEvent()
        else
          TriggerClientEvent("chatMessage", fPlayer, "", {0, 255, 0}, "^1계좌에 잔고가 부족합니다!")
        end
      end
    end
  end
