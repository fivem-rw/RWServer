vrp_blackjackC = {}
Tunnel.bindInterface("vrp_blackjack", vrp_blackjackC)
Proxy.addInterface("vrp_blackjack", vrp_blackjackC)
vRP = Proxy.getInterface("vRP")
vrp_blackjackS = Tunnel.getInterface("vrp_blackjack", "vrp_blackjack")
...
local defaultTimeLeft = 10
local timeLeft = defaultTimeLeft
...
vectorChairPos = {989.56066894531, 59.517021179199, 75.491386413574}

casinoBlackjackTablePositions = {
    [0] = {987.77701, 60.17849, 74.48293, 238.0},
    [1] = {991.2208, 58.07944, 74.48471, 58.0},
    [2] = {986.8749, 64.57017, 74.48395, 208.0},
    [3] = {983.6525, 57.50018, 74.4902, 288.0},
    [4] = {989.4973, 53.18717, 74.48966, 3.0},
    [5] = {995.0742, 60.43173, 74.49076, 103.0}
}

casinoBlackjackDealerPositions = {
    [0] = {988.48, 59.78, 74.49036, 56.5},
    [1] = {990.551171875, 58.467823028564, 74.49137878418, 235.67},
    [2] = {987.16626464844, 64.065032196045, 74.49137878418, 26.5},
    [3] = {984.48486328125, 57.745428466797, 74.49137878418, 106.53},
    [4] = {989.44992431641, 54.004062652588, 74.491287231445, 180.00},
    [5] = {994.30458984375, 60.242627716064, 74.491340637207, 281.61}
}

function vrp_blackjackC.notify(msg, type, timer)
    TriggerEvent(
...
Citizen.CreateThread(
    function()
        Citizen.Wait(1000)
        vrp_blackjackS.requestBlackjackTableData(
            {},
            function(newBlackjackTableData)
                print("newBlackjackTableData", newBlackjackTableData)
...
Citizen.CreateThread(
    function()
        ...
        for k, v in pairs(casinoBlackjackDealerPositions) do
            ...
            --dealerNetScene = NetworkCreateSynchronisedScene(v[1],v[2],v[3], 0.0, 0.0, v[4], 2, 0, 1, 1065353216, 0, 1065353216)
        ...
        for k, v in pairs(casinoBlackjackTablePositions) do
            local blackjackTable = GetClosestObjectOfType(v[1], v[2], v[3], 1.0, GetHashKey("vw_prop_casino_blckjack_01"), 0, 0, 0)
            SetObjectTextureVariant(blackjackTable, 2)
        end
    end
)
...
Citizen.CreateThread(
    function()
        while true do
            --print(sittingAtBlackjackTable, closestChair, closestChairDist, timeoutHowToBlackjack)
    ...
    function()
        while true do
            ...
            if drawCurrentHand then
                ...
                SetTextWrap(0, 0.87)
                SetTextEntry("STRING")
                AddTextComponentString("~w~당신의 카드 ~g~[" .. tostring(currentHand) .. "] ~w~ / 딜러의 카드 ~b~[" .. tostring(dealersHand) .. "]")
            ...
            if waitingForBetState then
                ...
                if IsControlJustPressed(0, 201) then --Submit bet [Enter]
                    ...
                    if tonumber(currentBetAmount) >= 0 then
                        vrp_blackjackS.setBlackjackBet({globalGameId, currentBetAmount, closestChair})
                        ...
                        vrp_blackjackC.notify(source, {"올바른 금액을 입력해주세요.", "error"})
                    end
                end
                if IsControlPressed(0, 32) then --Increase bet [pageup]
                    currentBetAmount = currentBetAmount + 1
                end
                if IsControlPressed(0, 33) then --Decrease bet [pagedown]
                    if currentBetAmount >= 1 then
                        currentBetAmount = currentBetAmount - 1
                    end
                end
            end
...
function vrp_blackjackC.successBlackjackBet()
            ...
            if closestChair ~= -1 and closestChairDist < 2 then
                if IsControlJustReleased(0, 38) then
                    if blackjackTableData[closestChair] == false then
                        vrp_blackjackS.requestSitAtBlackjackTable({closestChair})
                    end
                end
            end
            Wait(0)
...
function vrp_blackjackC.sitAtBlackjackTable(chair)
        ...
        while true do
            if sittingAtBlackjackTable and canExitBlackjack then
                ...
                if IsControlJustPressed(0, 202) and not waitingForSitDownState then --Leave seat [backspace]
                    ...
                    drawTimerBar = false
                    vrp_blackjackS.leaveBlackjackTable()
            ...
            if waitingForStandOrHitState and sittingAtBlackjackTable and blackjackGameInProgress then
                if IsDisabledControlJustPressed(0, 22) then --hit
                    ...
                    vrp_blackjackS.hitBlackjack({globalGameId, globalNextCardCount})
                    drawTimerBar = false
                ...
                if IsControlJustPressed(0, 202) then --stand
                    waitingForStandOrHitState = false
                    vrp_blackjackS.standBlackjack({globalGameId, globalNextCardCount})
                    drawTimerBar = false
                    standOrHitThisRound = true
                    declineCard()
                end
...
RMenu.Add("cmgblackjack", "instructions", RageUI.CreateMenu("", "test", 0, 350, "casinoui_cards_blackjack", "casinoui_cards_blackjack"))
RMenu:Get("cmgblackjack", "instructions"):SetSubtitle("~b~블랙잭")
RMenu.Add("cmgblackjack_high", "instructions", RageUI.CreateMenu("", "test", 0, 350, "casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high"))
RMenu:Get("cmgblackjack_high", "instructions"):SetSubtitle("~b~블랙잭")
        ...
        while true do
            if blackjackInstructional ~= nil then
                ...
                if waitingForBetState then
                    ...
                    DrawAdvancedNativeText(0.991, 0.935, 0.005, 0.0028, 0.29, "베팅:", 255, 255, 255, 255, 0, 0)
            ...
            if drawTimerBar then
                ...
                DrawAdvancedNativeText(1.013, 0.892, 0.005, 0.0028, 0.29, "시간:", 255, 255, 255, 255, 0, 0)
                if timeLeft < 10 then
...
function vrp_blackjackC.syncChipsPropBlackjack(betAmount, chairId)
    betBlackjack(betAmount, chairId)
end

function vrp_blackjackC.beginBetsBlackjack(gameID, tableId)
    globalGameId = gameID
    ...
    Citizen.CreateThread(
        function()
            ...
            --drawNativeNotification("지금은 베팅할 수 없습니다. 다음 턴을 기다리세요.")
            end
        end
    )
end

function vrp_blackjackC.beginCardGiveOut(gameId, cardData, chairId, cardIndex, gotCurrentHand, tableId)
...
function vrp_blackjackC.singleCard(gameId, cardData, chairID, nextCardCount, gotCurrentHand, tableId)
    dealerPed = getDealerFromTableId(tableId)
    startSingleDealing(chairID, dealerPed, gameId, cardData, nextCardCount + 1, gotCurrentHand)
end

function vrp_blackjackC.singleDealerCard(gameId, cardData, nextCardCount, gotCurrentHand, tableId)
    dealerPed = getDealerFromTableId(tableId)
    startSingleDealerDealing(dealerPed, gameId, cardData, nextCardCount + 1, gotCurrentHand, ((tableId + 1) * 4) - 1, tableId)
end

function vrp_blackjackC.standOrHit(gameId, chairId, nextCardCount, tableId)
    dealerPed = getDealerFromTableId(tableId)
    standOrHitThisRound = false
        ...
        Citizen.CreateThread(
            function()
                ...
                if not standOrHitThisRound and sittingAtBlackjackTable then
                    ...
                    vrp_blackjackS.standBlackjack({globalGameId, globalNextCardCount})
                    declineCard()
                    drawNativeNotification("스탠드/히트 시간 초과. 스탠딩 처리.")
                end
            end
        )
    else
...
function vrp_blackjackC.endStandOrHitPhase(chairId, tableId)
...
function vrp_blackjackC.bustBlackjack(chairID, tableId)
    --print("bustBlackjack!")
    dealerPed = getDealerFromTableId(tableId)
...
function vrp_blackjackC.flipDealerCard(gotCurrentHand, tableId, gameId)
    dealerPed = getDealerFromTableId(tableId)
    flipDealerCard(dealerPed, gotCurrentHand, tableId, gameId)
end
function vrp_blackjackC.dealerBusts(tableId)
    dealerPed = getDealerFromTableId(tableId)
    PlayAmbientSpeech1(dealerPed, "MINIGAME_DEALER_BUSTS", "SPEECH_PARAMS_FORCE_NORMAL_CLEAR", 1)
end
function vrp_blackjackC.blackjackLose(tableId)
    ...
    angryILost()
    canExitBlackjack = true
    drawNativeNotification("~r~당신은 졌다!")
    drawCurrentHand = false
    currentHand = 0
    dealersHand = 0
end
function vrp_blackjackC.blackjackPush(tableId)
    blackjackGameInProgress = false
    dealerPed = getDealerFromTableId(tableId)
    TaskPlayAnim(dealerPed, "anim_casino_b@amb@casino@games@blackjack@dealer", "reaction_impartial", 3.0, 1.0, -1, 2, 0, 0, 0, 0)
    annoyedIPushed()
    canExitBlackjack = true
    drawNativeNotification("~b~비겼습니다!")
    drawCurrentHand = false
    currentHand = 0
    dealersHand = 0
end
function vrp_blackjackC.blackjackWin(tableId)
    blackjackGameInProgress = false
    dealerPed = getDealerFromTableId(tableId)
    TaskPlayAnim(dealerPed, "anim_casino_b@amb@casino@games@blackjack@dealer", "reaction_good", 3.0, 1.0, -1, 2, 0, 0, 0, 0)
    happyIWon()
    canExitBlackjack = true
    drawNativeNotification("~g~당신은 이겼습니다!")
    drawCurrentHand = false
    currentHand = 0
    dealersHand = 0
end
function vrp_blackjackC.chipsCleanup(chairId, tableId)
...
function vrp_blackjackC.chipsCleanupNoAnim(chairId, tableId)
    for k, v in pairs(cardObjects) do
        if k == chairId then
            for k2, v2 in pairs(v) do
                DeleteEntity(v2)
...
function betChipsForNextHand(chipsAmount, chipsProp, something, chairID, someBool, zOffset)
    ...
    local modelHash = GetHashKey(chipsProp)
    ...
    local chipsObj = CreateObjectNoOffset(RWO, modelHash, chipsOffset.x, chipsOffset.y, chipsOffset.z, false, false, 1)
...
function getNewCardFromMachine(nextCard, chairId, gameId)
    ...
    local nextCardObj = CreateObjectNoOffset(RWO, nextCardHash, cardObjectOffset.x, cardObjectOffset.y, cardObjectOffset.z, false, false, 1)
...
function blackjack_func_69(id)
    ...
    --     return -122
    -- }
    if id ~= nil and casinoBlackjackTablePositions[id] ~= nil then
        return casinoBlackjackTablePositions[id][4]
    end
    return 0.0
end
...
function blackjack_func_70(id)
    ...
    elseif id ~= nil and casinoBlackjackTablePositions[id] ~= nil then
        return table.unpack(casinoBlackjackTablePositions[id])
    end
    return 0.0, 0.0, 0.0
end
...
function setupBlackjackMidBetScaleform(scaleform)
    ...
    PushScaleformMovieFunctionParameterInt(1)
    Button(GetControlInstructionalButton(2, 194, true)) -- The button to display
    ButtonMessage("스탠드") --BACKSPACE
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(4)
    Button(GetControlInstructionalButton(2, 22, true))
    ButtonMessage("히트") --Space
    PopScaleformMovieFunctionVoid()
