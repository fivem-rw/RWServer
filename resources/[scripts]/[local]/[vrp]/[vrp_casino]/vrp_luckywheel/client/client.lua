vrp_luckywheelC = {}
Tunnel.bindInterface("vrp_luckywheel", vrp_luckywheelC)
Proxy.addInterface("vrp_luckywheel", vrp_luckywheelC)
vRP = Proxy.getInterface("vRP")
vrp_luckywheelS = Tunnel.getInterface("vrp_luckywheel", "vrp_luckywheel")

local _wheel = nil
local _lambo = nil
local _isShowCar = false
local _movePos = vector3(948.98992919922, 63.164432525635, 75.991203308105)
local _wheelPos = vector3(948.5225, 63.42882, 75.23465)
local _baseWheelPos = vector3(948.5225, 63.42882, 74.98795)
local heading = 60.0
local Keys = {
    ["ESC"] = 322,
    ["BACKSPACE"] = 177,
    ["E"] = 38,
    ["ENTER"] = 18,
    ["LEFT"] = 174,
    ["RIGHT"] = 175,
    ["TOP"] = 27,
    ["DOWN"] = 173
}
local _isRolling = false
local renderDone = false
local carmodel = GetHashKey("grandgt18")
local vehPos = vector3(953.88177490234, 69.88818359375, 75.241249084473)

function showHowToBlackjack(flag)
    RageUI.Visible(RMenu:Get("cmgblackjack", "instructions"), flag)
end

function vrp_luckywheelC.notify(msg, type, timer)
    TriggerEvent(
        "pNotify:SendNotification",
        {
            text = msg,
            type = type or "success",
            timeout = timer or 3000,
            layout = "centerleft",
            queue = "global"
        }
    )
end

Citizen.CreateThread(
    function()
        while not renderDone do
            Citizen.Wait(1000)
            local dist = math.abs(#GetEntityCoords(PlayerPedId(-1)) - #vehPos)
            if dist < 5 then
                local model = GetHashKey("vw_prop_vw_luckywheel_02a")
                local baseWheelModel = GetHashKey("vw_prop_vw_luckywheel_01a")

                -- Base wheel
                RequestModel(baseWheelModel)
                while not HasModelLoaded(baseWheelModel) do
                    Citizen.Wait(0)
                end

                _basewheel = CreateObject(RWO, baseWheelModel, _baseWheelPos.x, _baseWheelPos.y, _baseWheelPos.z, false, false, true)
                SetEntityHeading(_basewheel, heading)
                SetModelAsNoLongerNeeded(baseWheelModel)

                -- Wheel
                RequestModel(model)

                while not HasModelLoaded(model) do
                    Citizen.Wait(0)
                end

                _wheel = CreateObject(RWO, model, _wheelPos.x, _wheelPos.y, _wheelPos.z, false, false, true)
                SetEntityHeading(_wheel, heading)
                SetModelAsNoLongerNeeded(model)

                NetworkRequestControlOfEntity(_wheel)

                local timeout = 2000
                while timeout > 0 and not NetworkHasControlOfEntity(_wheel) do
                    Wait(100)
                    timeout = timeout - 100
                end

                SetEntityAsMissionEntity(_wheel, true, true)

                local timeout = 2000
                while timeout > 0 and not IsEntityAMissionEntity(_wheel) do
                    Wait(100)
                    timeout = timeout - 100
                end

                -- Car
                RequestModel(carmodel)
                while not HasModelLoaded(carmodel) do
                    Citizen.Wait(0)
                end

                local vehicle = CreateVehicle(RWV, carmodel, vehPos.x, vehPos.y, vehPos.z, 0.0, false, false)

                SetVehicleOnGroundProperly(vehicle)
                SetModelAsNoLongerNeeded(carmodel)

                -- RequestCollisionAtCoord(1100.39, 220.09, -48.75)

                -- while not HasCollisionLoadedAroundEntity(vehicle) do
                --     RequestCollisionAtCoord(1100.39, 220.09, -48.75)
                --     Citizen.Wait(0)
                -- end

                -- SetVehRadioStation(vehicle, 'OFF')
                FreezeEntityPosition(vehicle, true)
                local _curPos = GetEntityCoords(vehicle)
                SetEntityCoords(vehicle, _curPos.x, _curPos.y, _curPos.z + 1, false, false, true, true)
                _lambo = vehicle

                renderDone = true
            end
        end
    end
)

Citizen.CreateThread(
    function()
        while true do
            if _lambo ~= nil then
                local _heading = GetEntityHeading(_lambo)
                local _z = _heading - 0.3
                SetEntityHeading(_lambo, _z)
            end
            Citizen.Wait(5)
        end
    end
)

RegisterNetEvent("esx_tpnrp_luckywheel:doRoll")
AddEventHandler(
    "esx_tpnrp_luckywheel:doRoll",
    function(_priceIndex)
        _isRolling = true
        SetEntityHeading(_wheel, heading)
        Citizen.CreateThread(
            function()
                local speedIntCnt = 1
                local rollspeed = 1.0
                local _winAngle = (_priceIndex - 1) * 18
                local _rollAngle = _winAngle + (360 * 28)
                local _midLength = (_rollAngle / 2)
                local intCnt = 0
                while speedIntCnt > 0 do
                    local retval = GetEntityRotation(_wheel, 2)
                    if _rollAngle > _midLength then
                        speedIntCnt = speedIntCnt + 1
                    else
                        speedIntCnt = speedIntCnt - 1
                        if speedIntCnt < 0 then
                            speedIntCnt = 0
                        end
                    end
                    intCnt = intCnt + 1
                    rollspeed = speedIntCnt / 10
                    local _y = retval.y - rollspeed
                    _rollAngle = _rollAngle - rollspeed
                    SetEntityHeading(_wheel, heading)
                    SetEntityRotation(_wheel, 0.0, _y, heading, 2, true)
                    Citizen.Wait(0)
                end
            end
        )
    end
)

RegisterNetEvent("esx_tpnrp_luckywheel:rollFinished")
AddEventHandler(
    "esx_tpnrp_luckywheel:rollFinished",
    function()
        _isRolling = false
    end
)

function doRoll()
    if not _isRolling then
        _isRolling = true
        local playerPed = PlayerPedId()
        local _lib = "anim_casino_a@amb@casino@games@lucky7wheel@female"
        if IsPedMale(playerPed) then
            _lib = "anim_casino_a@amb@casino@games@lucky7wheel@male"
        end
        local lib, anim = _lib, "enter_right_to_baseidle"

        if not HasAnimDictLoaded(lib) then
            RequestAnimDict(lib)

            while not HasAnimDictLoaded(lib) do
                Citizen.Wait(1)
            end
        end
        TaskGoStraightToCoord(playerPed, _movePos.x, _movePos.y, _movePos.z, 1.0, -1, heading, 0.0)
        local _isMoved = false
        while not _isMoved do
            local coords = GetEntityCoords(PlayerPedId())
            if coords.x >= (_movePos.x - 0.01) and coords.x <= (_movePos.x + 0.01) and coords.y >= (_movePos.y - 0.01) and coords.y <= (_movePos.y + 0.01) then
                _isMoved = true
            end
            Citizen.Wait(0)
        end
        TaskPlayAnim(playerPed, lib, anim, 8.0, -8.0, -1, 0, 0, false, false, false)
        while IsEntityPlayingAnim(playerPed, lib, anim, 3) do
            Citizen.Wait(0)
            DisableAllControlActions(0)
        end
        TaskPlayAnim(playerPed, lib, "enter_to_armraisedidle", 8.0, -8.0, -1, 0, 0, false, false, false)
        while IsEntityPlayingAnim(playerPed, lib, "enter_to_armraisedidle", 3) do
            Citizen.Wait(0)
            DisableAllControlActions(0)
        end
        TriggerServerEvent("esx_tpnrp_luckywheel:getLucky")
        TaskPlayAnim(playerPed, lib, "armraisedidle_to_spinningidle_high", 8.0, -8.0, -1, 0, 0, false, false, false)
    end
end

-- Menu Controls
Citizen.CreateThread(
    function()
        while true do
            Citizen.Wait(1)
            local coords = GetEntityCoords(PlayerPedId())

            if (GetDistanceBetweenCoords(coords, _wheelPos.x, _wheelPos.y, _wheelPos.z, true) < 1.5) and not _isRolling then
                showHowToBlackjack(true)
                if IsControlJustReleased(0, Keys["E"]) then
                    doRoll()
                end
            else
                showHowToBlackjack(false)
            end
        end
    end
)
