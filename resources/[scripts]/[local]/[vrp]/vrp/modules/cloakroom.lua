...
for k, v in pairs(cfg.cloakroom_types) do
  ...
  if not not_uniform then
    menu[lang.cloakroom.undress.title()] = {function(player, choice)
    ...
    function()
      for k, v in pairs(cfg.cloakrooms) do
        ...
        if cloakroom and menu then
          ...
          -- cloakroom
          vRPclient.addMarker(source, {x, y, z - 1, 1.2, 1.2, 0.2, 0, 125, 255, 125, 100, text})
          vRP.setArea(source, "vRP:cfg:cloakroom" .. k, x, y, z, 0.6, 1.5, cloakroom_enter, cloakroom_leave)
        end
