...
function vRP.sendServiceAlert(sender, service_name, x, y, z, msg)
  ...
  if ssa_last[sender] == nil then
    ssa_last[sender] = {}
  end
  if ssa_last[sender][service_name] == nil then
    ssa_last[sender][service_name] = os.time()
  elseif ssa_last[sender][service_name] > os.time() - 60 then
    vRPclient.notify(sender, {"~r~이미 같은 서비스를 호출했습니다. 잠시후 다시 시도해주세요."})
    return false
  else
    ssa_last[sender][service_name] = os.time()
  end
  ...
  if service then
    ...
    for k, v in pairs(players) do
      ...
      vRPclient.addBlip(
        v,
        {x, y, z, service.blipid, service.blipcolor, "(" .. service_name .. ") " .. msg},
        function(bid)
          SetTimeout(
            service.alert_time * 1000,
            function()
      ...
      if sender ~= nil then
        ...
        vRP.request(
          v,
          "[고유번호 : "..user_id.." | 닉네임 : "..name.."] " .. service_name .. "전화 받음, 수락하시겠습니까?",
          30,
          function(v, ok)
            if ok then -- take the call
              if not answered then
                ...
                if vname ~= nil and name ~= nil then
                  if service_name == "🚕 택시 호출" then
                    TriggerClientEvent("chatMessage", -1, "^*🚕 택시 알림", {255, 204, 51}, "" .. name .. "님, 기사님이 배정되었습니다. 잠시만 기다려주세요! 담당 택시 기사 :" .. vname) --r = 255, g = 204, b = 51
                  end
                  if service_name == "🚧 렉카 호출" then
                    TriggerClientEvent("chatMessage", -1, "^*🚧 퍼스트렉카 알림", {255, 204, 51}, "" .. name .. "님, 기사님이 배정되었습니다. 잠시만 기다려주세요! 담당 렉카 기사 :" .. vname) --r = 255, g = 204, b = 51
                  end                  
                  if service_name == "🔧 출장 수리" then
                    TriggerClientEvent("chatMessage", -1, "^*🔧 출장 수리 알림", {255, 204, 51}, "" .. name .. "님, 수리기사님이 배정되었습니다. 잠시만 기다려주세요! 담당 수리기사 :" .. vname) --r = 255, g = 204, b = 51
                  end
                  if service_name == "🔥 119 긴급" then
                    TriggerClientEvent("chatMessage", -1, "^*🚨 의료국 출동 알림", {255, 50, 50}, "" .. name .. "님, 119 신고가 접수되었습니다! 현장으로 즉시 출동하겠습니다! 담당 구급대원 :" .. vname) --r = 255, g = 204, b = 51
                  end
                --if service_name == "112 긴급" then
                -- TriggerClientEvent('chatMessage', -1, "^*�긴급출동 알림"  , { 255, 50, 50 }, ""..name.."님, 112 신고가 접수되었습니다! 현장으로 즉시 출동하겠습니다! 담당 경찰 :"..vname) --r = 255, g = 204, b = 51
                --end
                end
                answered = true
              else
...
local function ch_directory(player, choice)
  ...
  if user_id ~= nil then
    ...
    local ch_add = function(player, choice) -- add to directory
      vRP.prompt(
        player,
        lang.phone.directory.add.prompt_number(),
        "",
        function(player, phone)
          vRP.prompt(
            player,
            lang.phone.directory.add.prompt_name(),
            "",
    ...
    local ch_entry = function(player, choice) -- directory entry menu
      ...
      local ch_sendsms = function(player, choice) -- send sms to directory entry
        vRP.prompt(
          player,
          lang.phone.directory.sendsms.prompt({cfg.sms_size}),
          "",
          function(player, msg)
      ...
      emenu.onclose = function()
        ch_directory(player, lang.phone.directory.title())
      end
  ...
  if user_id ~= nil then
    ...
    for k, v in pairs(phone_sms) do
      ...
      menu["#" .. k .. " " .. from] = {
        ...
        lang.phone.sms.info({from, htmlEntities.encode(v[2])})
      }
    end
...
-- nest menu
service_menu.onclose = function(player)
  vRP.openMenu(player, phone_menu)
end
...
-- nest menu
announce_menu.onclose = function(player)
  vRP.openMenu(player, phone_menu)
end
        ...
        function(player, msg)
          ...
          if string.len(msg) > 10 and string.len(msg) < 1000 then
            if announce.price <= 0 or vRP.tryPayment(user_id, announce.price) then -- try to pay the announce
              vRPclient.notify(player, {lang.money.paid({format_num(announce.price)})})
...
AddEventHandler(
  ...
  function(type)
    ...
    if type == "ch_sa_taxi" then
      ch_service_alert(player, "🚕 택시 호출")
    elseif type == "ch_sa_tow" then
      ch_service_alert(player, "🚧 렉카 호출")
    elseif type == "ch_sa_repair" then
      ch_service_alert(player, "🔧 출장 수리")
    elseif type == "ch_sa_police" then
      ch_service_alert(player, "🚨 경찰 호출")
    elseif type == "ch_sa_ems" then
      ch_service_alert(player, "🔥 119 긴급")
    elseif type == "ch_announces_admin" then
      ch_announce_alert(player, "*🚨 관리자 공지배너")
    elseif type == "ch_announces_police" then
      ch_announce_alert(player, "*🚨 경찰 공지배너")
    elseif type == "ch_announces_ems" then
      ch_announce_alert(player, "*🚨 EMS 공지배너")
    end
  end
)