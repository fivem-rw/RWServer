...
local function ch_searchreg(player, choice)
  vRP.prompt(
    player,
    lang.police.pc.searchreg.prompt(),
    "",
    function(player, reg)
                      ...
                      vRP.getUserAddress(
                        ...
                        function(address)
                          ...
                          vRPclient.setDiv(
                            ...
                            {
                              "police_pc",
                              ".div_police_pc{ background-color: rgba(0,0,0,0.75); color: white; font-weight: bold; width: 500px; padding: 10px; margin: auto; margin-top: 150px; }",
                              content
                            }
                          )
                        end
            ...
            vRP.getUserBusiness(
              ...
              function(business)
                if identity and business then
                  vRP.request(
                    player,
                    lang.police.pc.closebusiness.request({identity.name, identity.firstname, business.name}),
                    15,
                    function(player, ok)
                      if ok then
...
local function ch_trackveh(player, choice)
  vRP.prompt(
    player,
    lang.police.pc.trackveh.prompt_reg(),
    "",
    function(player, reg) -- ask reg
      vRP.getUserByRegistration(
        reg,
        function(user_id)
          if user_id ~= nil then
            vRP.prompt(
              player,
              lang.police.pc.trackveh.prompt_note(),
              "",
              function(player, note) -- ask note
                -- begin veh tracking
                ...
                SetTimeout(
                  ...
                  function()
                    ...
                    if tplayer ~= nil then
                      vRPclient.getAnyOwnedVehiclePosition(
                        tplayer,
                        {},
                        function(ok, x, y, z)
...
local choice_getoutveh = {
  ...
  lang.police.menu.getoutveh.description()
}
...
local choice_askid = {
    ...
    vRPclient.getNearestPlayer(
      ...
      function(nplayer)
        ...
        if nuser_id ~= nil then
          ...
          vRP.request(
            nplayer,
            lang.police.menu.askid.request(),
            15,
            function(nplayer, ok)
              if ok then
                vRP.getUserIdentity(
                        ...
                        function(business)
                          ...
                          vRP.getUserAddress(
                            ...
                            function(address)
                              ...
                              local content = lang.police.identity.info({name, firstname, age, registration, phone, bname, bcapital, user_id, GetPlayerName(nplayer)})
                              vRPclient.setDiv(
                                player,
                                {
                                  "police_identity",
                              ...
                              vRP.request(
                                player,
                                lang.police.menu.askid.request_hide(),
                                1000,
                                function(player, ok)
  ...
  lang.police.menu.askid.description()
}
    ...
    vRPclient.getNearestPlayer(
      ...
      function(nplayer)
        ...
        if nuser_id ~= nil then
          ...
          vRP.request(
            nplayer,
            lang.police.menu.check.request(),
            15,
            function(nplayer, ok)
              if ok then
                ...
                vRPclient.getWeapons(
                  ...
                  function(weapons)
                    ...
                    items = items .. "<div class='checklist-row'>"
                    if data and data.inventory then
                      for k, v in pairs(data.inventory) do
                        local item = vRP.items[k]
                        if item then
                          items = items .. "<div class='checklist-item'>" .. item.name .. " (" .. format_num(v.amount) .. ")</div>"
                        end
                      end
                    end
                    items = items .. "</div>"
                    ...
                    local weapons_info = ""
                    weapons_info = weapons_info .. "<div class='checklist-row'>"
                    for k, v in pairs(weapons) do
                      weapons_info = weapons_info .. "<div class='checklist-item'>🔫 " .. k .. " (" .. v.ammo .. ")</div>"
                    end
                    weapons_info = weapons_info .. "</div>"
                    ...
                    vRPclient.setDiv(
                      player,
                      {
                        "police_check",
                        "",
                        lang.police.menu.check.info({items, weapons_info})
                      }
                    )
                    -- request to hide div
                    vRP.request(
...
local choice_seize_weapons = {
            ...
            vRPclient.getWeapons(
              ...
              function(weapons)
                for k, v in pairs(weapons) do -- display seized weapons
                  vRPclient.notify(player,"보유중인 무기가 모두 압수되었습니다.")
                  -- convert weapons to parametric weapon items
                 -- vRP.giveInventoryItem(user_id, "wbody|" .. k, 1, true)
                  if v.ammo > 0 then
                    vRPclient.notify(player,"보유중인 탄창이 모두 압수되었습니다.")
  ...
  lang.police.menu.seize.weapons.description()
}

local choice_seize_items = {
  function(player, choice)
    local player = player
    local user_id = vRP.getUserId(player)
    if user_id ~= nil then
  ...
  lang.police.menu.seize.items.description()
}

local choice_check_driver = {
  function(player, choice)
    local player = player
    local user_id = vRP.getUserId(player)
    if user_id ~= nil then
        ...
        function(nplayer)
          ...
          if nuser_id ~= nil then
           local name = GetPlayerName(nplayer)
            if vRP.CheckInventoryItem(nuser_id,'driver',1,"운전면허증") then
            vRPclient.notify(player,{"[ 결 과 ]\n"..name.."님의\n~g~운전면허증~w~이\n확인 되었습니다."})
            vRPclient.notify(nplayer,{"당신의 ~g~운전면허증~w~이 조회되었습니다."})
          else
            vRPclient.notify(player,{"[ 결 과 ]\n"..name.."님은\n~r~운전면허증~w~을\n보유하고 있지 않습니다.."})
            vRPclient.notify(nplayer,{"당신은 ~r~운전면허증~w~이 없습니다."})
            end
            else
              vRPclient.notify(player, {lang.common.no_player_near()})
              end
  ...
  lang.police.menu.check.driver.description()
}
...
local choice_jail = {
  ...
  lang.police.menu.jail.description()
}
...
local choice_fine = {
      ...
      vRPclient.getNearestPlayer(
        ...
        function(nplayer)
          ...
          if nuser_id ~= nil then
            ...
            -- build fine menu
            local menu = {
              name = lang.police.menu.fine.title(),
              css = {top = "75px", header_color = "rgba(0,125,255,0.75)"}
            }
  ...
  lang.police.menu.fine.description()
}
          ...
          function(player, choice)
            vRP.buildMenu(
              ...
              function(menu)
                ...
                if vRP.hasPermission(user_id, "police.seize.items") then
                  menu[lang.police.menu.check.driver.title()] = choice_check_driver
                end
...
local function build_client_points(source)
  ...
  for k, v in pairs(cfg.pcs) do
    ...
    vRPclient.addMarker(source, {x, y, z - 1, 1.5, 1.5, 0.2, 0, 125, 255, 125, 150})
...
AddEventHandler(
  ...
  function(type)
    ...
    if type == "choice_check" then
      if vRP.hasPermission(user_id, "police.check") then
        choice_check[1](source, "")
      end
    elseif type == "choice_seize_weapons" then
      if vRP.hasPermission(user_id, "police.seize.weapons") then
        choice_seize_weapons[1](source, "")
      end
    elseif type == "choice_seize_items" then
      if vRP.hasPermission(user_id, "police.seize.items") then
        choice_seize_items[1](source, "")
      end
    elseif type == "choice_askid" then
      if vRP.hasPermission(user_id, "police.askid") then
        choice_askid[1](source, "")
      end
    elseif type == "choice_putinveh" then
      if vRP.hasPermission(user_id, "police.putinveh") then
        choice_putinveh[1](source, "")
      end
    elseif type == "choice_getoutveh" then
      if vRP.hasPermission(user_id, "police.getoutveh") then
        choice_getoutveh[1](source, "")
      end
    end
  end
)
