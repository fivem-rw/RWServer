...
function vRP.addUserGroup(user_id, group)
  if not vRP.hasGroup(user_id, group) then
    ...
    if group and ngroup then
      ...
      if user_id ~= nil and group ~= nil and gtype ~= nil then
...
function vRP.getGroupTopLevel(user_id)
  local user_groups = vRP.getUserGroups(user_id)
  local topLevel = 0
  for k, v in pairs(user_groups) do
    local groupdef = groups[k]
    if groupdef and groupdef._config and groupdef._config.level then
      if groupdef._config.level > topLevel then
        topLevel = groupdef._config.level
      end
    end
  end
...
function vRP.getGroupLevel(name)
  local groupdef = groups[name]
  if groupdef and groupdef._config and groupdef._config.level then
    return groupdef._config.level
  end
  return 0
end

function vRP.isExistGroup(name)
  local groupdef = groups[name]
  if groupdef then
    return true
  end
...
function vRP.hasPermissionByGroup(name, perm)
  local groupdef = groups[name]
  if groupdef then
    for _, v in pairs(groupdef) do
      if v == perm then
        return true
      end
    end
  ...
  if user_id ~= nil then
    for k, v in pairs(selectors) do
      ...
      if gcfg and menu then
        ...
        vRPclient.addMarker(source, {x, y, z - 1, 1.5, 1.5, 0.2, 255, 154, 24, 125, 150, text})
