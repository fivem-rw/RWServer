...
local function tr_remove_player(tr, player) -- remove player from transforming
  ...
  -- onstop
  if tr.itemtr.onstop then
    tr.itemtr.onstop(player, recipe)
  end
end
...
local function tr_add_player(tr, player, recipe) -- add player to transforming
  ...
  vRP.closeMenu(player)
  vRPclient.setProgressBar(
    player,
    {"vRP:tr:" .. tr.name, "center", recipe .. "...", tr.itemtr.r, tr.itemtr.g, tr.itemtr.b, 0}
  )
  ...
  for k, v in pairs(tr.players) do
    ...
    if v and user_id ~= nil then -- for each player transforming
      ...
      if tr.units > 0 and recipe then -- check units
        ...
        local new_weight =
          vRP.getInventoryWeight(user_id) + vRP.computeItemsWeight(out_witems) - vRP.computeItemsWeight(in_witems)
...
function vRP.setItemTransformer(name, itemtr)
  ...
  tr.menu = {
    name = itemtr.name,
    css = {top = "75px", header_color = "rgba(" .. itemtr.r .. "," .. itemtr.g .. "," .. itemtr.b .. ",0.75)"}
  }
...
local function transformers_tick()
  SetTimeout(
    0,
    function()
      -- error death protection for transformers_tick()
...
local function transformers_regen()
  for k, tr in pairs(transformers) do
    ...
    if tr.units >= tr.itemtr.max_units then
...
local function ch_informer_buy(player, choice)
  ...
  if user_id ~= nil and tr ~= nil then
    if vRP.tryPayment(user_id, price) then
      ...
      vRPclient.notify(player, {lang.money.paid({format_num(price)})})
...
local function informer_placement_tick()
  ...
  for k, v in pairs(vRP.rusers) do
    ...
    vRPclient.setNamedBlip(
      player,
      {"vRP:informer", x, y, z, cfg.informer.blipid, cfg.informer.blipcolor, lang.itemtr.informer.title()}
    )
    vRPclient.setNamedMarker(player, {"vRP:informer", x, y, z - 1, 1.5, 1.5, 0.2, 0, 255, 125, 125, 150})
