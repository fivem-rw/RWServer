  ...
  for gtype, mitems in pairs(market_types) do
              ...
              if amount > 0 then
                ...
                if new_weight <= vRP.getInventoryMaxWeight(user_id) then
                  ...
                  if vRP.tryPayment(user_id, amount * taxprice) then
                    ...
                    MySQL.execute("vRP/add_tax", {statecoffers = amount * tax})
                    vRPclient.notify(player, {"리얼월드 소비세 8%"})
                  else
    ...
    for k, v in pairs(mitems) do
      ...
      if item then
        ...
        market_menu[item.name] = {market_choice, "소비세 포함 " .. lang.market.info({v + tax, item.description})}
      end
    end
      ...
      function()
        for k, v in pairs(markets) do
          ...
          if group and menu then -- check market type
            ...
            vRP.setArea(source, "vRP:market" .. k, x, y, z, 0.6, 1.5, market_enter, market_leave)
          end
