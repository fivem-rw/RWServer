...
function vRP.closeMenu(source, force)
  vRPclient.closeMenu(source, {force})
end
...
function tvRP.promptResult(text)
  ...
  if source ~= nil and prompt ~= nil then
    prompts[source] = nil
    if source then
      prompt(source, text)
    end
...
SetTimeout(
  10000,
  function()
    -- wait for vRP.addStaticMenuChoices calls
  ...
  if user_id ~= nil then
    for k, v in pairs(cfg.static_menus) do
      ...
      if menu and smenu then
        ...
        vRPclient.addMarker(source, {x, y, z - 1, 1.5, 1.5, 0.2, 255, 226, 0, 125, 150})
