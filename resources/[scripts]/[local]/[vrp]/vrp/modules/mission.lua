  ...
  if user_id ~= nil then
    ...
    if tmpdata.mission_step ~= nil then -- if in a mission
      ...
      if tmpdata.mission_step > #tmpdata.mission_data.steps then -- check mission end
        ...
        local blipid = 1
        local blipcolor = 5
        local onleave = function(player, area)
        end
        if step.blipid then
          blipid = step.blipid
        end
        ...
        vRPclient.setDivContent(
          ...
          {
            "mission",
            lang.mission.display({tmpdata.mission_data.name, tmpdata.mission_step - 1, #tmpdata.mission_data.steps, step.text})
          }
        )
        ...
        vRPclient.setNamedBlip(
          player,
          {
            ...
            blipid,
            blipcolor,
            lang.mission.blip({tmpdata.mission_data.name, tmpdata.mission_step, #tmpdata.mission_data.steps})
          },
          function(id)
        ...
        -- map trigger
        vRPclient.setNamedMarker(player, {"vRP:mission", x, y, z - 1, 1.5, 1.5, 0.2, 255, 226, 0, 125, 150})
...
vRP.registerMenuBuilder(
  ...
  function(add, data)
    ...
    if user_id ~= nil then
      ...
      -- build admin menu
      choices[lang.mission.cancel.title()] = {
