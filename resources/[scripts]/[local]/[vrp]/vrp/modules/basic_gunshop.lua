...
for gtype, weapons in pairs(gunshop_types) do
    ...
    if weapon then
      ...
      vRPclient.getWeapons(
        ...
        function(weapons)
          ...
          vRP.prompt(
            player,
            lang.gunshop.prompt_ammo({choice}),
            "",
            function(player, amount)
              ...
              if amount >= 0 then
                ...
                if user_id ~= nil and vRP.tryPayment(user_id, total) then
                  ...
                  vRPclient.notify(player, {lang.money.paid({format_num(total)})})
                else
  ...
  for k, v in pairs(weapons) do
    if k ~= "_config" then -- ignore config property
      ...
      gunshop_menu[v[1]] = {gunshop_choice, lang.gunshop.info({format_num(v[2]), format_num(v[3]), v[4]})} -- add description
    end
  end
      ...
      function()
        for k, v in pairs(gunshops) do
          ...
          if group and menu then
            ...
            vRPclient.addMarker(source, {x, y, z - 1, 1.2, 1.2, 0.2, 255, 0, 0, 125, 100})

            vRP.setArea(source, "vRP:gunshop" .. k, x, y, z, 0.6, 1.5, gunshop_enter, gunshop_leave)
          end
