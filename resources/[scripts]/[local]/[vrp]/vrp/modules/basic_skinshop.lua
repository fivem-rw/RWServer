      ...
      function(old_custom)
        ...
        local ondrawable = function(player, choice, mod)
          if mod == 0 then -- tex variation
            ...
            vRPclient.getDrawableTextures(
              source,
              {parts[choice], drawable[1]},
              function(n)
                if n == nil then
                  n = 0
                end
                ...
                if textures[choice][1] == nil or textures[choice][1] >= n then
          ...
          vRPclient.getCustomization(
            ...
            function(custom)
              ...
              for k, v in pairs(custom) do
                ...
                if v[1] ~= old[1] then
                  price = price + cfg.drawable_change_price
                end -- change of drawable
                if v[2] ~= old[2] then
                  price = price + cfg.texture_change_price
                end -- change of texture
              end

              if vRP.tryPayment(user_id, price) then
                if price > 0 then
                  vRPclient.notify(source, {lang.money.paid({format_num(price)})})
                end
              else
    ...
    Citizen.CreateThread(
      function()
        for k, v in pairs(skinshops) do
          local parts, x, y, z, text, args = table.unpack(v)
          local isRestArea = false
          local isSpecialSkin = false
          if args then
            if args[1] then
              isSpecialSkin = true
              if not vRP.hasGroup(user_id, "specialskin") then
                isRestArea = true
              end
            elseif args[2] then
              isSpecialSkin = true
              if not vRP.hasPermission(user_id, "mask.shop") then
                isRestArea = true
              end
            end
          end

          if not isRestArea then
            ...
            if isSpecialSkin then
              vRPclient.addMarker(source, {x, y, z - 1, 1.2, 1.2, 0.2, 255, 0, 0, 125, 100, text})
            else
            ...
            vRP.setArea(source, "vRP:skinshop" .. k, x, y, z, 0.6, 1.5, skinshop_enter, skinshop_leave)
          end
