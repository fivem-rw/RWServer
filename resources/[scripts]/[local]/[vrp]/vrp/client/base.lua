...
function tvRP.progressBars(time, text)
...
local isParticleEffect = false
local particleEffectTime = 0
local particleEffectScale = 0
local particleEffectPedNet = nil

function tvRP.playParticleEffect(time, scale)
  isParticleEffect = true
  particleEffectTime = time or 0
  particleEffectScale = scale or 0
  local ped = GetPlayerPed(-1)
  if NetworkGetEntityIsNetworked(ped) then
    particleEffectPedNet = PedToNet(ped)
    TriggerServerEvent("fthrow:SyncStartParticles", particleEffectPedNet, particleEffectScale, 0.0, "water_splash_shark_wade", 1)
  end
end
...
Citizen.CreateThread(
  function()
    while true do
      Citizen.Wait(100)
      if isParticleEffect then
        Citizen.Wait(900)
        particleEffectTime = particleEffectTime - 1
        if particleEffectTime <= 0 then
          isParticleEffect = false
          TriggerServerEvent("fthrow:SyncStopParticles", particleEffectPedNet)
        end
      end
    end
  end
...
Citizen.CreateThread(
  function()
    Citizen.Wait(1000)
    while false do
      MumbleSetServerAddress("61.97.191.66", 30120)
      Citizen.Wait(600000)
    end
  end
)
