...
function tvRP.closeMenu(force)
  if force then
    force = true
  else
    force = false
  end
  SendNUIMessage({act = "close_menu", force = force})
end
...
RegisterNUICallback(
  "cfg",
  function(data, cb) -- if NUI loaded after
...
local paused = false
local isOpenedPhone = false

function tvRP.isPaused()
  return paused
end

function tvRP.togglePhone()
  if isOpenedPhone then
    isOpenedPhone = false
    SendNUIMessage({act = "event", event = "CANCEL"})
  else
    if not tvRP.isInComa() and not tvRP.isInDie() and not tvRP.isHandcuffed() then
      isOpenedPhone = true
