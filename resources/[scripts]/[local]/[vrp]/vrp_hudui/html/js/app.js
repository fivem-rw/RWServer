...
$(document).ready(function (e) {
	...
	setInterval(function () {
		$('#datetime_status').text(moment().format('YYYY-MM-DD a hh:mm:ss'))
	}, 1000)
})

window.onload = function () {
	var eventCallback = {
		toggle_ui: function (data) {
			if (data.data == true) {
				$('#ui').show()
			}
			else {
		...
		update_players: function (data) {
			$('#player_num').html(data.data)
		},
		update_ping: function (data) {
			var value = parseInt(data.data)
			$('#ping_status').html(value)
			if (value >= 80) {
				$('#ping_status_e').html('💔')
			}
			else if (value >= 50) {
		...
		update_fps: function (data) {
			var value = parseInt(data.data)
			$('#fps_status').html(value)
			if (value >= 30) {
				$('#fps_status_e').html('📗')
			}
			else if (value >= 20) {
		...
		seatbelt: function (data) {
			if (data.status == true) {
				eventCallback.sound('sounds/warning.ogg', { volume: '0.10', loop: true }, '#sounds1');
				document.querySelector('#container').style.display = 'block';
			}
			else {
				eventCallback.soundStop('#sounds1')
		...
		ui: function (data) {
			...
			if (config.showCreditMoney == true) { document.querySelector('#credit').style.display = 'block'; }
		...
		setVoiceDistance: function (data) {
			...
			voiceIdWithClasses.remove('whisper', 'normal', 'shout', 'lock');
		...
		updateVehicle: function (data) {
			...
			if (data.status == true) {
				...
				$('#status').addClass('inveh')
				$('#desctext').addClass('inveh')
				...
				if (vehicleInfo.classList.contains('active')) {
					...
					$('#status').removeClass('inveh')
					$('#desctext').removeClass('inveh')
		...
		sound: function (file = null, args = null, id = '#sounds') {
