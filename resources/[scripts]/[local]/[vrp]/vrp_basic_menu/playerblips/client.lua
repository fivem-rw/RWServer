...
local showblipKY = false
local showsprite = false
local block_run = false

vRP = Proxy.getInterface("vRP")

RegisterNetEvent("showBlips:ky")
AddEventHandler(
	"showBlips:ky",
	function()
		showblipKY = not showblipKY
		if showblipKY then
			showsprite = true
		else
			showsprite = false
...
AddEventHandler(
	...
	function()
		...
		if showblip then
			showsprite = true
			vRP.notify({"~w~빌립스 모드 ~g~ON"})
		else
			showsprite = false
			vRP.notify({"~w~빌립스 모드 ~r~OFF"})
		end
	end
)
		...
		while true do
			...
			for _, id in ipairs(GetActivePlayers()) do
				if GetPlayerPed(id) ~= mped then
					...
					if (showblip or showblipKY) and IsEntityVisible(ped) and GetEntityAlpha(ped) > 60 then
	...
	function()
		while true do
			...
			if GetDistanceBetweenCoords(1689.693, 2586.405, 51.2557, GetEntityCoords(playerPed)) < 195.0 then
				if showblipKY == false and block_run == false then
					TriggerServerEvent("blips:checkper")
					TriggerServerEvent("block:checkper")
					Citizen.Wait(1000)
				end
			else
				if showblipKY == true then
					showsprite = false
					showblipKY = false
					vRP.notify({"~r~[ 교정본부 퇴장 ]~w~\n위치표시가 ~r~비활성화~w~ 되었습니다."})
				end
