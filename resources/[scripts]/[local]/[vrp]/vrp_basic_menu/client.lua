...
function vRPbm.spawnVehicle(model)
	...
	if HasModelLoaded(mhash) then
		...
		local nveh = CreateVehicle(RWV, mhash, x, y, z + 0.5, GetEntityHeading(GetPlayerPed(-1)), true, false) -- added player heading
		...
		vRP.notify({"~g~차량 소환 완료."})
	else
		vRP.notify({"~r~차량 모델 없음."})
	end
end
...
function vRPbm.getNearestVehicle(radius)
	...
	if IsPedSittingInAnyVehicle(ped) then
		...
		if not IsEntityAVehicle(veh) then
			veh = GetClosestVehicle(x + 0.0001, y + 0.0001, z + 0.0001, radius + 5.0001, 0, 4 + 2 + 1)
		end -- cars
		return veh
	end
end
...
function vRPbm.deleteVehicleInFrontOrInside(offset)
	...
	if IsEntityAVehicle(veh) then
		...
		vRP.notify({"~g~차량 제거 완료."})
	else
		vRP.notify({"~r~차량과 거리가 멉니다."})
	end
end
...
Citizen.CreateThread(
	function()
		while true do
			...
			if not IsControlPressed(0, 21) and IsControlJustReleased(1, 117) and not IsPedInAnyVehicle(GetPlayerPed(-1), false) then
				BMserver.drag()
			end
			if not IsControlPressed(0, 21) and IsControlJustReleased(1, 108) and not IsPedInAnyVehicle(GetPlayerPed(-1), false) then
				BMserver.cuff()
			end
			Citizen.Wait(0)
		end
