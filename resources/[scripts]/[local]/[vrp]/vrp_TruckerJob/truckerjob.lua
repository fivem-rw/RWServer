vRP_truckerJobS = Tunnel.getInterface("vRP_truckerJob", "vRP_truckerJob")
...
local Truck = {"MAN", "ACTROS"}
local Trailer = {"TANKER", "TRAILERS", "TRAILERS2", "TRAILERLOGS"}

local MissionData = {
    [0] = {1212.4463, 2667.4351, 38.79, 500000}, --x,y,z,money
    [1] = {-410.6452331543,1177.8696289063,325.64175415039, 800000}, --x,y,z,money
    [2] = {2574.5144, 328.5554, 108.45, 1000000},
    [3] = {-2565.0894, 2345.8904, 33.06, 1500000},
    [4] = {1706.7966, 4943.9897, 42.16, 2000000},
    [5] = {196.5617, 6631.0967, 31.53, 2500000},
    [6] = {-567.66717529297,5356.8525390625,70.21849822998, 3500000},
    [7] = {2824.3383789063,1605.6512451172,24.660732269287, 4000000}
}
local MISSION = {}
MISSION.start = false
...
function init()
    ...
    Citizen.Trace("트럭 맵 아이콘이 추가되었습니다.")
   -- GUI.loaded = true
end

RegisterNetEvent("okcok")
AddEventHandler("okcok", function()
  GUI.showMenu = true
  Citizen.Wait(3000)
end)
...
function tick()
    ...
    elseif( MISSION.start == true ) then
        ...
        if( IsEntityAttached(MISSION.trailer) and text1 == false) then
            TriggerEvent("mt:missiontext", "화물을 실어 ~g~목적지~w~로 가세요!", 10000)
            text1 = true
        elseif( not IsEntityAttached(MISSION.trailer) and text2 == false ) then
            TriggerEvent("mt:missiontext", "~o~트레일러를 ~w~연결하세요!", 15000)
            text2 = true
        end
        Wait(2000)
        ...
        if ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and  not IsEntityAttached(MISSION.trailer)) then
            TriggerEvent("mt:missiontext", "다시 ~b~트럭 기지~w~로 돌아가 새로운 미션을 시작하세요."..currentMission[4], 15000)
            MISSION.removeMarker()
            MISSION.destroytrailer()
            vRP_truckerJobS.success({currentMission[4]})
            clear()
        elseif ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and IsEntityAttached(MISSION.trailer) ) then
            TriggerEvent("mt:missiontext", "도착했습니다. ~o~트레일러~w~를 해제하려면 ~r~H~w~를 누르세요.", 15000)
        end
...
function MISSION.spawnTruck()
    ...
    MISSION.truck = CreateVehicle(RWV, MISSION.hashTruck, TruckingTruck[rand][1], TruckingTruck[rand][2], TruckingTruck[rand][3], 0.0, true, false)
...
function MISSION.spawnTrailer()
	...
	MISSION.trailer = CreateVehicle(RWV, MISSION.hashTrailer, TruckingTrailer[rand][1], TruckingTrailer[rand][2], TruckingTrailer[rand][3], 0.0, true, false)
    SetVehicleOnGroundProperly(MISSION.trailer)
...
function GUI.drawStartText()
    TriggerEvent("mt:missiontext", "트럭 미션을 시작하려면 넘버패드 +키를 누르세요.", 500)
...
function GUI.init()
    GUI.loaded = true
    GUI.addTitle("당신은 트럭 기사 미션을 보고 있습니다.", 0.425, 0.19, 0.45, 0.07 )
    GUI.addDesc("[사용방법]", 0.575, 0.375, 0.15, 0.30 )

    --menu, title, function, position
    GUI.addButton(0, "유조 탱크 트레일러", GUI.optionMisson, 0.35, 0.25, 0.3, 0.05 )
    GUI.addButton(0, "나가기", GUI.exit, 0.35, 0.50, 0.3, 0.05 )
    
    GUI.buttonCount = 0
    
    GUI.addButton(1, "미션 1 [ 보상 500,000원 ]", GUI.mission, 0.35, 0.25, 0.3, 0.05)
    GUI.addButton(1, "미션 2 [ 보상 800,000원 ]", GUI.mission, 0.35, 0.30, 0.3, 0.05)
    GUI.addButton(1, "미션 3 [ 보상 1,000,000원 ]", GUI.mission, 0.35, 0.35, 0.3, 0.05)
    GUI.addButton(1, "미션 4 [ 보상 1,500,000원 ]", GUI.mission, 0.35, 0.40, 0.3, 0.05)
    GUI.addButton(1, "미션 5 [ 보상 2,000,000원 ]", GUI.mission, 0.35, 0.45, 0.3, 0.05)
    GUI.addButton(1, "미션 6 [ 보상 2,500,000원 ]", GUI.mission, 0.35, 0.50, 0.3, 0.05)
    GUI.addButton(1, "미션 7 [ 보상 3,500,000원 ]", GUI.mission, 0.35, 0.55, 0.3, 0.05)
    GUI.addButton(1, "미션 8 [ 보상 4,000,000원 ]", GUI.mission, 0.35, 0.60, 0.3, 0.05)
    GUI.addButton(1, "나가기", GUI.exit, 0.35, 0.70, 0.3, 0.05)
end
...
function GUI.updateSelectionMenu(menu)
    ...
    elseif( IsControlPressed(0, Keys["ENTER"]) ) then
        if( type(GUI.button[menu][GUI.selected[menu]]["func"]) == "function" ) then
            ...
            Citizen.Trace("\n 불러오기를 실패 하였습니다. - 메뉴 선택 : "..GUI.selected[menu].." \n")
        end
        GUI.time = 0
    end
    local i = 0
