RegisterNetEvent("Police:getWeapon")
AddEventHandler("Police:getWeapon", function(hash,type)
	...
	if DoesEntityExist(closestPed) and closestPedDst <= 5.0 then
		...
		TriggerServerEvent("Police:giveWeapon", weaponHash)
	end
...
local Exists = nil
RegisterNetEvent('Police:pedExists')
AddEventHandler('Police:pedExists', function(ret)
    Exists = ret
end)
...
RefreshPed = function(spawn)
	...
	Exists = nil
	TriggerServerEvent('Police:pedExists')
	while Exists == nil do 
		Wait(0)
	end
