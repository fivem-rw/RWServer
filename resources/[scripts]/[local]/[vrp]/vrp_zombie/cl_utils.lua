...
function Utils.SendNotification(icon, type, sender, title, text, sound)
...
function Utils.CreatePed(model, pedType, pos, heading)
    if IsModelAPed(model) then
        ...
        local ped = CreatePed(RWP, pedType, model, pos.x, pos.y, pos.z, heading, true)
...
function Utils.CreateVehicle(model, pos, heading)
    if IsModelAVehicle(model) then
        ...
        local veh = CreateVehicle(RWV, model, table.unpack(pos), heading, true)
...
function Utils.CreateProp(model, pos, dynamic, placeOnGround)
    if IsModelValid(model) then
        ...
        local prop = CreateObject(RWO, model, x, y, GetGroundZFor_3dCoord(x, y, z), true, false, dynamic)
