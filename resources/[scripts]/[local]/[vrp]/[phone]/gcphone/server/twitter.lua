...
function TwitterPostTweet(username, password, message, sourcePlayer, realUser, cb)
  getUser(
    ...
    function(user)
      ...
      MySQL.Async.insert(
        "INSERT INTO twitter_tweets (`authorId`, `message`, `realUser`) VALUES(@authorId, @message, @realUser);",
        {
          ["@authorId"] = user.id,
          ["@message"] = message,
          ["@realUser"] = realUser
        ...
        function(id)
          MySQL.Async.fetchAll(
            "SELECT * from twitter_tweets WHERE id = @id",
            {
              ["@id"] = id
            },
            function(tweets)
          ...
          MySQL.Async.fetchAll(
            ...
            function(row)
              if (row[1] == nil) then
                ...
                MySQL.Async.execute(
                  "DELETE FROM twitter_likes WHERE id = @id",
                  {
                    ["@id"] = row[1].id
                  },
                  function(newrow)
...
function TwitterCreateAccount(username, password, avatarUrl, cb)
  MySQL.Async.insert(
    "INSERT IGNORE INTO twitter_accounts (`username`, `password`, `avatar_url`) VALUES(@username, @password, @avatarUrl)",
    {
      ["username"] = username,
      ["password"] = password,
      ["avatarUrl"] = avatarUrl
