...
AddTextEntry(controllerMessageLoweredCar, "~INPUT_CONTEXT~키를 눌러서 베드를 ~y~올릴 ~w~수 있습니다.~n~~INPUT_DETONATE~키를 눌러서 차량의 고정을 ~r~해제 ~w~할 수 있습니다.")
AddTextEntry(controllerMessageLoweredNoCar, "~INPUT_CONTEXT~키를 눌러서 베드를 ~y~올릴 ~w~수 있습니다.~n~~INPUT_DETONATE~키를 눌러서 차량을 ~g~고정 ~w~하세요.")
AddTextEntry(controllerMessageRaised, "~INPUT_CONTEXT~키를 눌러서 베드를 ~y~내릴 ~w~수 있습니다.")
...
function network(id)
    ...
    local c = 0
    while not NetworkGetEntityIsNetworked(id) and c < 100 do
        c = c + 1
        Wait(100)
    end
    ...
    if NetworkGetEntityIsNetworked(id) then
        print("^2[network()]: Networked entity")
    else
        print("^1[network()]: Failed to network entity")
    end
    ...
    function()
        ...
        while true do
            ...
            if veh and DoesEntityExist(veh) and GetEntityModel(veh) == GetHashKey("flatbed3") and NetworkHasControlOfEntity(veh) then
                ...
                if not DecorExistOn(veh, "flatbed3_bed") or DecorGetInt(veh, "flatbed3_bed") == 0 and f then
                    DecorSetInt(veh, "flatbed3_bed", 0)
                    local bed = CreateObjectNoOffset(RWO, "inm_flatbed_base", pos, true, 1, 1)
                    f = false
                    log("GENERATING BED")
                    if bed and DoesEntityExist(bed) then
                        local bedNet = network(bed)
                        ...
                        if drawMarker(controllerPos) then
                            ...
                            if IsControlJustPressed(0, 47) then
                                if attached then
                                    ...
                                    local newCar = getVehicleInDirection(bedPos + vector3(0.0, 0.0, -2.25), bedPos + vector3(0.0, 0.0, 2.25))
                                    if newCar and tonumber(newCar) ~= 0 then
                                        local carPos = GetEntityCoords(newCar, false)
                                        NetworkRequestControlOfEntity(newCar)
                                        local c = 0
                                        while not NetworkHasControlOfEntity(newCar) and c < 100 do
                                            c = c + 1
                                            Wait(0)
                                        end
                                        if c < 100 then
                                            local zOffset = carPos.z - bedPos.z - 0.50
                                            if carPos.z <= bedPos.z then
                                                zOffset = 0.8
                                            end
                                            AttachEntityToEntity(newCar, bed, 0, attachmentOffset[1] + vector3(0.0, 0.0, zOffset), attachmentOffset[2], 0, 0, false, 0, 0, 1)
                                            car = newCar
