...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		LSCMenu:addPurchase("Repair vehicle", round(3000000 + 6000000 * damage, 0), "전신 수리 및 엔진 서비스.")
		...
		if chassis then
			...
			AddMod(46, LSCMenu.categories.Chassis, "DOORS", "Doors", "", true)
			-- windows
...
function LSCMenu:onSelectedIndexChanged(name, button)
	...
	elseif m == "neon color" then
		if myveh.neon then
			...
			LSCMenu:showNotification("~r~네온 조명이 설치되어 있지 않습니다.")
		end
	elseif m == "xenon color" then
		if myveh.mods[22].mod == 1 then
			SetVehicleHeadlightsColour(veh, button.xenon)
		else
			LSCMenu:showNotification("~r~제논 헤드 라이트가 설치되어 있지 않습니다.")
		end
...
function LSCMenu:onButtonSelected(name, button)
	...
	if not button.purchased then
		...
		LSCMenu:showNotification("~r~이미 착용된 부품입니다.")
	end
end
...
function CanPurchase(price, canpurchase)
	if canpurchase then
		if LSCMenu.currentmenu == "main" then
			LSCMenu:showNotification("~r~차량을 먼저 수리 후 튜닝이 가능합니다.")
		else
			LSCMenu:showNotification("~g~해당 부품을 구매했습니다.")
		end
		return true
	else
		LSCMenu:showNotification("~r~부품을 구매할 수 없습니다.")
		return false
	end
end
