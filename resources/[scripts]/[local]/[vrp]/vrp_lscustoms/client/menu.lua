					...
					if i >= menu.from and i <= menu.to then
						if menu.selected == i then
							...
							elseif btn.price ~= nil then	
								if btn.price == 0 then
									...
									if not btn.purchased then
										drawRightMenuText(format_num(btn.price).." 원",cfg.font,0,x + (width/2.1),ty,scale,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
									else
							...
							elseif btn.price ~= nil then
								if btn.price ~= 0 then
									if not btn.purchased then
										drawRightMenuText(format_num(btn.price).." 원",cfg.font,0,x + (width/2.1),ty,scale,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
									else
