...
local garages = {
	...
	[4] = {
		...
		inside = {x = 1174.823, y = 2637.807, z = 37.045, heading = 181.19}
	},
	[5] = {
		locked = false,
		...
		inside = {x = 108.842, y = 6628.447, z = 31.072, heading = 45.504}
	},
	[6] = {
		locked = false,
...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		SetIbuttons(
			{
				{GetControlInstructionalButton(1, LSCMenu.config.controls.menu_back, 0), "뒤"},
				{GetControlInstructionalButton(1, LSCMenu.config.controls.menu_select, 0), "선택"},
				{GetControlInstructionalButton(1, LSCMenu.config.controls.menu_up, 0), "위"},
				{GetControlInstructionalButton(1, LSCMenu.config.controls.menu_down, 0), "아래"},
				{GetControlInstructionalButton(1, LSCMenu.config.controls.menu_left, 0), "왼쪽"},
				{GetControlInstructionalButton(1, LSCMenu.config.controls.menu_right, 0), "오른쪽"}
			},
			0
		)
		...
		AddMod(0, LSCMenu.categories, "스포일러", "스포일러", "Increase downforce.", true)
		AddMod(3, LSCMenu.categories, "스커트", "스커트", "Enhance your vehicle's look with custom side skirts.", true)
		AddMod(4, LSCMenu.categories, "배기", "배기", "Customized sports exhausts.", true)
		AddMod(6, LSCMenu.categories, "그릴", "그릴", "Improved engine cooling.", true)
		AddMod(7, LSCMenu.categories, "후드", "후드", "Enhance car engine cooling.", true)
		AddMod(8, LSCMenu.categories, "휀더", "휀더", "Enhance body paneling with custom fenders.", true)
		AddMod(10, LSCMenu.categories, "루프", "루프", "Lower your center of gravity with lightweight roof panels.", true)
		AddMod(12, LSCMenu.categories, "브레이크", "브레이크", "Increase stopping power and eliminate brake fade.", true)
		AddMod(13, LSCMenu.categories, "변속기", "변속기", "Improved acceleration with close ratio transmission.", true)
		AddMod(14, LSCMenu.categories, "크락션", "크락션", "Custom air horns.", true)
		AddMod(15, LSCMenu.categories, "서스펜션", "서스펜션", "Upgrade to a sports oriented suspension setup.", true)
		AddMod(16, LSCMenu.categories, "내구도", "내구도", "Protect your car's occupants with military spec composite body panels.", true)
		AddMod(18, LSCMenu.categories, "터보", "터보", "Reduced lag turbocharger.", false)

		if chassis then
			LSCMenu.categories:addSubMenu("샷시", "Chassis", nil, true)
			...
			AddMod(44, LSCMenu.categories.Chassis, "루프스코프", "루프스코프", "", true) --roof scoops
			AddMod(45, LSCMenu.categories.Chassis, "연료통", "연료통", "", true)
			AddMod(46, LSCMenu.categories.Chassis, "문", "문", "", true)
			AddMod(5, LSCMenu.categories.Chassis, "롤케이지", "롤케이지", "Stiffen your chassis with a rollcage.", true)
		end

		LSCMenu.categories:addSubMenu("엔진", "Engine", nil, true)
		AddMod(39, LSCMenu.categories.Engine, "엔진블록", "엔진블록", "Custom engine block casings.", true)
		AddMod(40, LSCMenu.categories.Engine, "캠 커버", "캠 커버", "Optional cam covers.", true)
		AddMod(41, LSCMenu.categories.Engine, "브레이스바", "브레이스바", "A selection of support struts.", true)
		AddMod(11, LSCMenu.categories.Engine, "엔진튜닝", "엔진튜닝", "Increases horsepower.", true)

		if interior then
			LSCMenu.categories:addSubMenu("인테리어", "Interior", "Products for maximum style and comfort.", true)
			--LSCMenu.categories.Interior:addSubMenu("TRIM", "Trim","A selection of interior designs.", true)
		AddMod(27, LSCMenu.categories.Interior, "트림", "트림", "", true)
		...
		AddMod(29, LSCMenu.categories.Interior, "대시보드", "대시보드", "Custom control panel designs.", true)
		AddMod(30, LSCMenu.categories.Interior, "다이얼", "다이얼", "Customize the look of your dials.", true)
		AddMod(31, LSCMenu.categories.Interior, "도어", "도어", "Install door upgrades.", true)
		AddMod(32, LSCMenu.categories.Interior, "시트", "시트", "Options where style meets comfort.", true)
		AddMod(33, LSCMenu.categories.Interior, "스티어링휠", "스티어링휠", "Customize the link between you and your vehicle.", true)
		AddMod(34, LSCMenu.categories.Interior, "쉬프터 레버", "쉬프터 레버", "", true)
		AddMod(35, LSCMenu.categories.Interior, "Plaques", "Plaques", "", true)
		AddMod(36, LSCMenu.categories.Interior, "스피커", "스피커", "", true)
		AddMod(37, LSCMenu.categories.Interior, "트렁크", "트렁크", "", true)
		end

		LSCMenu.categories:addSubMenu("번호판", "Plates", "Decorative identification.", true)
		LSCMenu.categories.Plates:addSubMenu("번호판", "License", "", true)
		...
		AddMod(25, LSCMenu.categories.Plates, "번호판 홀더", "번호판 홀더", "", true) --
		...
		if bumper then
			LSCMenu.categories:addSubMenu("범퍼", "Bumpers", "Custom front and rear bumpers.", true)
			if fbumper then
				AddMod(1, LSCMenu.categories.Bumpers, "프론트 범퍼", "프론트 범퍼", "Custom front bumpers.", true)
			end
			if rbumper then
				AddMod(2, LSCMenu.categories.Bumpers, "리어 범퍼", "리어 범퍼", "Custom rear bumpers.", true)
			end
		end

		local m = LSCMenu.categories:addSubMenu("라이트", "Lights", "Improved night time visibility.", true)
		AddMod(22, LSCMenu.categories.Lights, "헤드라이트", "헤드라이트", nil, false)
		if not IsThisModelABike(GetEntityModel(veh)) then
			neon = m:addSubMenu("네온 키트", "Neon kits", nil, true)
			neon:addSubMenu("네온 레이아웃", "Neon layout", nil, true)
		...
		respray = LSCMenu.categories:addSubMenu("도색", "Respray", "Transforms vehicle appearance.", true)
		pcol = respray:addSubMenu("기본 컬러", "Primary color", nil, true)
		pcol:addSubMenu("크롬", "Chrome", nil, true)
		...
		pcol:addSubMenu("클래식", "Classic", nil, true)
		...
		pcol:addSubMenu("매트", "Matte", nil, true)
		...
		pcol:addSubMenu("메탈릭", "Metallic", nil, true)
		...
		pcol:addSubMenu("메탈", "Metals", nil, true)
		...
		scol = respray:addSubMenu("추가 색상", "Secondary color", nil, true)
		scol:addSubMenu("크롬", "Chrome", nil, true)
		...
		scol:addSubMenu("클래식", "Classic", nil, true)
		...
		scol:addSubMenu("매트", "Matte", nil, true)
		for n, c in pairs(LSC_Config.prices.chrome2.colors) do
		...
		scol:addSubMenu("메탈릭", "Metallic", nil, true)
		...
		scol:addSubMenu("메탈", "Metals", nil, true)
		...
		LSCMenu.categories:addSubMenu("휠", "Wheels", "Custom rims, tires and colors.", true)
		wtype = LSCMenu.categories.Wheels:addSubMenu("휠 종류", "Wheel type", "Custom rims in all styles and sizes.", true)
		if IsThisModelABike(GetEntityModel(veh)) then
			fwheels = wtype:addSubMenu("프론트 휠", "Front wheel", nil, true)
			...
			bwheels = wtype:addSubMenu("리어 휠", "Back wheel", nil, true)
			...
			sportw = wtype:addSubMenu("스포츠", "Sport", nil, true)
			...
			musclew = wtype:addSubMenu("머슬", "Muscle", nil, true)
			...
			lowriderw = wtype:addSubMenu("로우라이더", "Lowrider", nil, true)
			...
			suvw = wtype:addSubMenu("SUV", "Suv", nil, true)
			...
			offroadw = wtype:addSubMenu("오프로드", "Offroad", nil, true)
			...
			tunerw = wtype:addSubMenu("튜너", "Tuner", nil, true)
			...
			hughendw = wtype:addSubMenu("하이엔드", "Highend", nil, true)
			for n, w in pairs(LSC_Config.prices.highendwheels) do
				local btn = hughendw:addPurchase(w.name, w.price)
				btn.wtype = w.wtype
				btn.modtype = 23
				btn.mod = w.mod
			end
		...
		m = LSCMenu.categories.Wheels:addSubMenu("휠 색상", "Wheel color", "Custom wheel colors.", true)
		for n, c in pairs(LSC_Config.prices.wheelcolor.colors) do
			local btn = m:addPurchase(c.name, LSC_Config.prices.wheelcolor.price)
			btn.colorindex = c.colorindex
		end

		m = LSCMenu.categories.Wheels:addSubMenu("휠 악세사리", "Wheel accessories", "Bulletproof tires and custom burnout smoke.", true)
		...
		m = LSCMenu.categories:addSubMenu("창문", "Windows", "A selection of tinted windows.", true)
		btn = m:addPurchase("None")
		btn.tint = false
							...
							if LSC_Config.oldenter then
								...
								if GetDistanceBetweenCoords(outside.x, outside.y, outside.z, GetEntityCoords(ped)) <= f(5) then
									...
									if not tableContains(LSC_Config.ModelBlacklist, GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
										...
										if pos.locked then
											...
											if not LSC_Config.lock then
												if IsControlJustPressed(1, 201) then
													...
													drawTxt("~b~ENTER~w~를 눌러서 튜닝 시작", 0, 1, 0.5, 0.8, 1.0, 255, 255, 255, 255)
												end
											else
												drawTxt("~r~차량이 잠겼습니다. 잠시만 기다려주세요.", 0, 1, 0.5, 0.8, 1.0, 255, 255, 255, 255)
											end
										else
											if IsControlJustPressed(1, 201) then
												inside = true
										...
										drawTxt("~r~이 차량은 튜닝할 수 없습니다.", 0, 1, 0.5, 0.8, 1.0, 255, 255, 255, 255)
									end
								end
							else
								...
								if math.abs(GetEntityHeading(veh) - outside.heading) <= 90 and IsVehicleStopped(veh) and GetDistanceBetweenCoords(outside.x, outside.y, outside.z, GetEntityCoords(ped)) <= f(5) then
									...
									if not tableContains(LSC_Config.ModelBlacklist, GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
										...
										drawTxt("~r~이 차량은 튜닝할 수 없습니다.", 4, 1, 0.5, 0.8, 1.0, 255, 255, 255, 255)
									end
								end
							end
						end
...
AddEventHandler(
	"LSC:buttonSelected",
	function(name, button, canpurchase)
		name = name:lower()
		local m = LSCMenu.currentmenu
		local price = button.price or 0
		local veh = myveh.vehicle
		...
		if
			mname == "liveries" or mname == "hydraulics" or mname == "horn" or mname == "tank" or mname == "ornaments" or mname == "arch cover" or mname == "aerials" or mname == "roof scoops" or mname == "doors" or mname == "roll cage" or mname == "engine block" or mname == "cam cover" or
...
AddEventHandler(
	...
	function(vehicle, modifications)
...
function CheckPurchases(m)
	...
	elseif
		name == "armor" or name == "suspension" or name == "transmission" or name == "brakes" or name == "engine tunes" or name == "roof" or name == "fenders" or name == "hood" or name == "grille" or name == "roll cage" or name == "exhausts" or name == "skirts" or name == "rear bumpers" or
	...
	elseif
		name == "tank" or name == "ornaments" or name == "arch cover" or name == "aerials" or name == "roof scoops" or name == "doors" or name == "roll cage" or name == "engine block" or name == "cam cover" or name == "strut brace" or name == "trim design" or name == "ornametns" or name == "dashboard" or
			name == "dials" or
			name == "seats" or
...
local function AddBlips()
	for i, pos in ipairs(garages) do
		...
		if i == 5 then
			...
			AddTextComponentString("자동차 튜닝샵")
	...
	function()
		while true do
			...
			if LSCMenu:isVisible() then
				DrawIbuttons()
				--Draw the scaleform if menu is visible
