...
"use strict";
if(typeof process==='undefined'||!process.version||process.version.indexOf('v0.')===0||process.version.indexOf('v1.')===0&&process.version.indexOf('v1.8.')!==0){module.exports={nextTick:nextTick};}else{module.exports=process;}function nextTick(fn,arg1,arg2,arg3){if(typeof fn!=='function'){throw new TypeError('"callback" argument must be a function');}var len=arguments.length;var args,i;switch(len){case 0:case 1:return process.nextTick(fn);case 2:return process.nextTick(function afterTickOne(){fn.call(null,arg1);});case 3:return process.nextTick(function afterTickTwo(){fn.call(null,arg1,arg2);});case 4:return process.nextTick(function afterTickThree(){fn.call(null,arg1,arg2,arg3);});default:args=new Array(len-1);i=0;while(i<args.length){args[i++]=arguments[i];}return process.nextTick(function afterTick(){fn.apply(null,args);});}}
...
"use strict";
module.exports=Transform;var Duplex=__webpack_require__(5);var util=Object.create(__webpack_require__(7));util.inherits=__webpack_require__(8);util.inherits(Transform,Duplex);function afterTransform(er,data){var ts=this._transformState;ts.transforming=false;var cb=ts.writecb;if(!cb){return this.emit('error',new Error('write callback called multiple times'));}ts.writechunk=null;ts.writecb=null;if(data!=null)this.push(data);cb(er);var rs=this._readableState;rs.reading=false;if(rs.needReadable||rs.length<rs.highWaterMark){this._read(rs.highWaterMark);}}function Transform(options){if(!(this instanceof Transform))return new Transform(options);Duplex.call(this,options);this._transformState={afterTransform:afterTransform.bind(this),needTransform:false,transforming:false,writecb:null,writechunk:null,writeencoding:null};this._readableState.needReadable=true;this._readableState.sync=false;if(options){if(typeof options.transform==='function')this._transform=options.transform;if(typeof options.flush==='function')this._flush=options.flush;}this.on('prefinish',prefinish);}function prefinish(){var _this=this;if(typeof this._flush==='function'){this._flush(function(er,data){done(_this,er,data);});}else{done(this,null,null);}}Transform.prototype.push=function(chunk,encoding){this._transformState.needTransform=false;return Duplex.prototype.push.call(this,chunk,encoding);};Transform.prototype._transform=function(chunk,encoding,cb){throw new Error('_transform() is not implemented');};Transform.prototype._write=function(chunk,encoding,cb){var ts=this._transformState;ts.writecb=cb;ts.writechunk=chunk;ts.writeencoding=encoding;if(!ts.transforming){var rs=this._readableState;if(ts.needTransform||rs.needReadable||rs.length<rs.highWaterMark)this._read(rs.highWaterMark);}};Transform.prototype._read=function(n){var ts=this._transformState;if(ts.writechunk!==null&&ts.writecb&&!ts.transforming){ts.transforming=true;this._transform(ts.writechunk,ts.writeencoding,ts.afterTransform);}else{ts.needTransform=true;}};Transform.prototype._destroy=function(err,cb){var _this2=this;Duplex.prototype._destroy.call(this,err,function(err2){cb(err2);_this2.emit('close');});};function done(stream,er,data){if(er)return stream.emit('error',er);if(data!=null)stream.push(data);if(stream._writableState.length)throw new Error('Calling transform done when ws.length != 0');if(stream._transformState.transforming)throw new Error('Calling transform done when still transforming');return stream.push(null);}
...
const{parseUrl}=__webpack_require__(9);const Profiler=__webpack_require__(34);const Logger=__webpack_require__(35);const MySQL=__webpack_require__(36);const{typeCast,safeInvoke,sanitizeInput,sanitizeTransactionInput}=__webpack_require__(90);const logger=new Logger(global.GetConvar('mysql_debug_output','console'));const profiler=new Profiler(logger,{trace:global.GetConvarInt('mysql_debug',0),slowQueryWarningTime:global.GetConvarInt('mysql_slow_query_warning',100)});const config=JSON.parse(global.LoadResourceFile('ghmattimysql','config.json'));const configFromString=parseUrl(global.GetConvar('mysql_connection_string','mysql://localhost/fivem'));const mysql=new MySQL(config||configFromString,logger,profiler);const currentResourceName=global.GetCurrentResourceName();global.exports('scalar',(query,parameters,callback,resource)=>{const invokingResource=resource||global.GetInvokingResource();let sql=query;let values=parameters;let cb=callback;[sql,values,cb]=sanitizeInput(sql,values,cb);mysql.execute({sql,values,typeCast},invokingResource).then(result=>{safeInvoke(cb,result&&result[0]?Object.values(result[0])[0]:null);return true;}).catch(()=>false);});global.exports('execute',(query,parameters,callback,resource)=>{const invokingResource=resource||global.GetInvokingResource();let sql=query;let values=parameters;let cb=callback;[sql,values,cb]=sanitizeInput(sql,values,cb);mysql.execute({sql,values,typeCast},invokingResource).then(result=>{safeInvoke(cb,result);return true;}).catch(()=>false);});global.exports('transaction',(querys,values,callback,resource)=>{const invokingResource=resource||global.GetInvokingResource();let sqls=[];let cb=callback;[sqls,cb]=sanitizeTransactionInput(querys,values,cb);mysql.beginTransaction(connection=>{if(!connection)safeInvoke(cb,false);const promises=[];sqls.forEach(element=>{promises.push(mysql.execute({sql:element.query,values:element.values},invokingResource,connection));});mysql.commitTransaction(promises,connection,result=>{safeInvoke(cb,result);});});});global.onNet(`${currentResourceName}:request-data`,()=>{const src=global.source;global.emitNet(`${currentResourceName}:update-resource-data`,src,profiler.profiles.resources);global.emitNet(`${currentResourceName}:update-time-data`,src,profiler.profiles.executionTimes);global.emitNet(`${currentResourceName}:update-slow-queries`,src,profiler.profiles.slowQueries);});
...
exports.ChangeUser=__webpack_require__(46);exports.Handshake=__webpack_require__(72);exports.Ping=__webpack_require__(73);exports.Query=__webpack_require__(20);exports.Quit=__webpack_require__(82);exports.Sequence=__webpack_require__(3);exports.Statistics=__webpack_require__(83);
...
var Timers=__webpack_require__(71);module.exports=Timer;function Timer(object){this._object=object;this._timeout=null;}Timer.prototype.active=function active(){if(this._timeout){if(this._timeout.refresh){this._timeout.refresh();}else{Timers.active(this._timeout);}}};Timer.prototype.start=function start(msecs){this.stop();this._timeout=Timers.setTimeout(this._onTimeout.bind(this),msecs);};Timer.prototype.stop=function stop(){if(this._timeout){Timers.clearTimeout(this._timeout);this._timeout=null;}};Timer.prototype._onTimeout=function _onTimeout(){return this._object._onTimeout();};
...
"use strict";
module.exports=PassThrough;var Transform=__webpack_require__(26);var util=Object.create(__webpack_require__(7));util.inherits=__webpack_require__(8);util.inherits(PassThrough,Transform);function PassThrough(options){if(!(this instanceof PassThrough))return new PassThrough(options);Transform.call(this,options);}PassThrough.prototype._transform=function(chunk,encoding,cb){cb(null,chunk);};
...
function typeCast(field,next){let dateString='';switch(field.type){case'DATETIME':case'DATETIME2':case'TIMESTAMP':case'TIMESTAMP2':case'NEWDATE':case'DATE':dateString=field.string();if(field.type==='DATE')dateString+=' 00:00:00';return new Date(dateString).getTime();case'TINY':if(field.length===1){return field.string()!=='0';}return next();case'BIT':return Number(field.buffer()[0]);default:return next();}}function mysqlConvertLegacyFormat(query,parameters){let sql=query;const params=[];sql=sql.replace(/@(\w+)/g,(txt,match)=>{let returnValue=txt;if(Object.prototype.hasOwnProperty.call(parameters,txt)){params.push(parameters[txt]);returnValue='?';}else if(Object.prototype.hasOwnProperty.call(parameters,match)){params.push(parameters[match]);returnValue='?';}return returnValue;});return{sql,params};}function prepareLegacyQuery(query,parameters){let sql=query;let params=parameters;if(params!==null&&typeof params==='object'&&!Array.isArray(params)){({sql,params}=mysqlConvertLegacyFormat(sql,params));}return[sql,params];}function sanitizeInput(query,parameters,callback){let sql=query;let params=parameters;let cb=callback;if(typeof parameters==='function'){cb=parameters;}[sql,params]=prepareLegacyQuery(query,params);if(!Array.isArray(params)){params=[];}return[sql,params,cb];}function safeInvoke(callback,args){if(typeof callback==='function'){setImmediate(()=>{callback(args);});}}function prepareTransactionLegacyQuery(querys){const sqls=querys;sqls.forEach((element,index)=>{const[query,values]=prepareLegacyQuery(element.query,element.values);sqls[index]={query,values:Array.isArray(values)?values:[]};});return sqls;}function sanitizeTransactionInput(querys,params,callback){let sqls=[];let cb=callback;if(!querys.every(element=>typeof element==='string'))sqls=querys;else{const values=typeof params==='function'?[]:params;querys.forEach(element=>{sqls.push({query:element,values});});}if(typeof params==='function')cb=params;sqls=prepareTransactionLegacyQuery(sqls);return[sqls,cb];}module.exports={typeCast,safeInvoke,sanitizeInput,sanitizeTransactionInput};
